...
describe('status --all', () => {
  ...
  it('rejects --all combined with --change', async () => {
    ...
    expect(getOutput(result)).toContain('互斥');
  });
  ...
  it('honors the JSON null-shape when --all and --change are combined', async () => {
    ...
    expect(Array.isArray(json.status)).toBe(true);
    expect(json.status[0].severity).toBe('error');
    expect(json.status[0].message).toContain('互斥');
  });
  ...
  it('prints one text block per change with --all', async () => {
    ...
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain('变更：first-change');
    expect(result.stdout).toContain('变更：second-change');
  ...
  it('gives every change in the sweep its own next-step line', async () => {
    ...
    expect(result.stdout).toContain(
      'Next: openspec-cn instructions specs --change "first-change" --json'
    );
    expect(result.stdout).toContain(
      'Next: openspec-cn instructions specs --change "second-change" --json'
    );
  });
  ...
  it('prints no next-step line for a change that failed to load', async () => {
    ...
    expect(result.stdout).toContain('Next: openspec-cn instructions specs --change "good-change"');
  ...
  it('exits 1 in text mode when a change fails to load, still printing the others', async () => {
    ...
    expect(result.stdout).toContain('变更：good-change');
    expect(result.stdout).toContain('2/4 个制品已完成');
  });
  ...
  describe('a namespace folder holding nested changes (#1846)', () => {
    ...
    it('reports the nesting instead of an artifact plan for work that is not there', async () => {
      ...
      expect(result.stdout).toContain('"mobile" 不是一个变更');
      expect(result.stdout).not.toContain('变更：mobile');
    ...
    it('carries the diagnostic in --json in place, preserving the envelope', async () => {
      ...
      expect(entry.status).toEqual([
        expect.objectContaining({
          severity: 'error',
          code: 'change_error',
          message: expect.stringContaining('"mobile" 不是一个变更'),
        }),
      ]);
    });
  });
  ...
  describe('--schema interaction', () => {
    ...
    it('fails with the null-shape when --schema names an unknown schema', async () => {
      ...
      expect(Array.isArray(json.status)).toBe(true);
      expect(json.status[0].severity).toBe('error');
      expect(json.status[0].message).toContain('未找到 Schema');
    });
