...
// The CN build's CLI name (`openspec-cn`) matches the bare-name shape but is the
// executable, not a skill, so it is excluded explicitly.
const CN_CLI_NAME = 'openspec-cn';
...
function expectInstalledReferences(content: string, workflows: readonly string[], label: string): void {
  for (const match of content.matchAll(/[/@]opsx[:-]([\w-]+)|\b(openspec-[\w-]+)/g)) {
    if (match[0] === CN_CLI_NAME) continue;
