...
describe('change status policy - next step', () => {
  ...
  describe('an artifact is ready', () => {
    it('names that artifact instructions command', () => {
      expect(resolveNextStep(input())).toEqual({
        command: 'openspec-cn instructions specs --change "add-dark-mode" --json',
        sentence:
          '运行 openspec-cn instructions specs --change "add-dark-mode" --json，然后再编写该制品。',
      });
    });
    ...
    it('picks the FIRST ready artifact, not the last', () => {
      ...
      expect(step?.command).toContain('openspec-cn instructions specs ');
    ...
    it('never names a skipped artifact', () => {
      ...
      expect(step?.command).toContain('openspec-cn instructions design ');
  ...
  describe('planning is complete', () => {
    it('names the apply instructions command', () => {
      ...
      expect(step).toEqual({
        command: 'openspec-cn instructions apply --change "add-dark-mode" --json',
        sentence:
          '所有规划制品均已完成。运行 openspec-cn instructions apply --change "add-dark-mode" --json 以检查实现进度。',
      });
    });
  });
