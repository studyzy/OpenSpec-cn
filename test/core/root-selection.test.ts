...
describe('resolveOpenSpecRoot', () => {
  ...
  describe('declared store fallback (3.2)', () => {
    ...
    it('starts every declaration failure with a prefix the project check names', async () => {
      ...
      // Every declaration failure ends with one of these two phrasings, and
      // the guard names them so the agent can tell a broken declaration from
      // "never initialized".
      const prefixes = ['中声明', '中的 store 声明无效'];
      for (const prefix of prefixes) {
        expect(PROJECT_ROOT_GUARD).toContain(`\`${prefix}\``);
      }
      const namedByGuard = (message: string) => prefixes.some((prefix) => message.includes(prefix));
