...
describe('an unparseable global config', () => {
  ...
  it('warns about it once, however many times it is read', async () => {
    ...
    const warnings = consoleErrorSpy.mock.calls.filter((call) => String(call[0]).includes('JSON 无效'));
  ...
  describe('openspec config', () => {
    ...
    it.each([
      ...
      expect(errorOutput()).toContain('openspec-cn config edit');
    });
  ...
  it('is left byte-identical by a read-only CLI command', async () => {
    ...
    expect(result.stderr.match(/JSON 无效/g)).toHaveLength(1);
  }, 120_000);
});
