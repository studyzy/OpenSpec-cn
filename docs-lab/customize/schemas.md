...
- **templates/** 改变每个文档的骨架。在 tasks 模板里添加一个章节，之后每个新建的 tasks.md 都会以它开头。把 `#` 标题保留在第一行：制品会继承它，因此每个生成的文件打开时都是一个带标题的文档。
- **schema.yaml** 改变工作流本身：存在哪些制品、每个制品首先依赖什么，以及 Agent 创建它时收到的指令。
