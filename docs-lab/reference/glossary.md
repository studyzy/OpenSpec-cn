...
OpenSpec 复用了一些在 git、CI 和 Agent 工具中含义不同的词。每一行给出 OpenSpec 中的含义，最后一列在可用时链接到更多细节。

| 术语 | 定义 | 更多 |
|---|---|---|
| **Apply** | 实现变更提案中的任务。Skill：`openspec-apply-change`。 | [应用变更](skills.md#openspec-apply-change) |
| **Archive** | 完成一个变更提案：将其增量合并到主 spec，并把它的文件夹移到 `openspec/changes/archive/`。 | [快速开始](../start/quickstart.md) |
| **Artifact** | 变更提案内的规划文档：`proposal.md`、delta specs、`design.md`、`tasks.md`。不是构建产物。 | [制品](schemas/spec-driven/index.md#artifacts) |
| **Capability** | 你系统的一个行为领域。每个能力在 `openspec/specs/<capability>/spec.md` 有一个 spec。 | [能力](schemas/spec-driven/index.md#proposalmd) |
| **Change proposal** | 一个工作单元：`openspec/changes/<name>/` 下的一个文件夹，存放其规划制品。常简称为"变更"。不是 git 提交。 | [Propose](../start/quickstart.md#step-2-propose) |
...
| **Explore** | 在提议之前，先与 Agent 一起把想法想清楚。不写代码。Skill：`openspec-explore`。 | [探索想法](skills.md#openspec-explore) |
| **Fast-forward** | 一次性创建带有全部规划制品的变更提案，即可直接进入实现。Skill：`openspec-ff-change`。不是 git 的 fast-forward。 | [技能](skills.md) |
| **Legacy workflow** | OPSX 之前的 `/openspec:*` 命令。 |  |
| **Loop** | 变更提案经历的一个循环：explore、propose、review、apply、archive。 | [快速开始](../start/quickstart.md) |
| **Main specs** | `openspec/specs/` 目录树：你系统当前已达成一致的行为。归档会把增量合并进去。尚无 spec 的能力会从其 `ADDED` 需求获得一份。 | [归档](../start/quickstart.md#step-5-archive) |
...
| **OPSX** | 当前的 OpenSpec 工作流系统，以及它安装的命令前缀（`/opsx:`）。 |  |
...
| **Spec** | 描述某个能力当前如何行为的文件，位于 `openspec/specs/<capability>/spec.md`。 | [归档](../start/quickstart.md#step-5-archive) |
| **spec-driven** | 默认 schema：proposal，然后 delta specs，然后 design，然后 tasks。 | [spec-driven](schemas/spec-driven/index.md) |
| **Store** | 注册在你机器上的独立 OpenSpec 仓库，用于跨仓库规划。不是数据存储。 | [Stores（beta）](../multi-repo/stores.md) |
| **Sync** | 在不归档的情况下，将已实现的增量合并到主 spec。Skill：`openspec-sync-specs`。 | [技能](skills.md) |
| **Template** | schema 提供给每个制品的起始内容。 | [Schemas](../customize/schemas.md) |
| **Update** | 作为技能（`openspec-update-change`）：修订变更提案的规划制品。作为 CLI 命令（`openspec-cn update`）：刷新 OpenSpec 已安装的文件。 | [更新变更](skills.md#openspec-update-change)、[CLI](cli.md) |
