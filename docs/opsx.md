...
/opsx:explore
```
深入思考想法、调查问题、比较选项。无需固定结构——只是一个思考伙伴。当见解明晰时，可过渡到 `/opsx:propose`（默认）或 `/opsx:new`/`/opsx:ff`（扩展）。
...
| **依赖关系** | 无（一次性创建全部） | 有向无环图（DAG）+ 拓扑排序 |
| **状态** | 基于阶段的心智模型 | 文件系统存在性检查 |
| **定制** | 编辑源码，重新构建 | 创建 schema.yaml |
| **迭代** | 阶段锁定 | 流动式，随时编辑 |
| **编辑器支持** | 工具特定的配置器/适配器 | 统一的技能目录 |
