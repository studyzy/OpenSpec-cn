...
**Explore（`/opsx:explore`）.** 思考伙伴命令。它会阅读你的代码库、比较各种方案，并把一个模糊的想法澄清为一份具体的计划。它从不写代码，也不会写其他任何东西，除非你要求它把这次探索捕获为一个变更，或者当它主动提出时你表示同意。只要你有一个问题但还没有计划，它就是推荐的起点。参见 [先探索](explore.md)。
