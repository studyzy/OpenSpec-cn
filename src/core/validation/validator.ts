...
export class Validator {
      ...
      for (const { id: specId, specFile } of discoveredSpecs) {
        ...
        for (const orphan of plan.orphanedRequirements) {
          ...
          issues.push({
            ...
            path: entryPath,
            line: orphan.line,
            message: `需求 "${orphan.name}" ${where}，这不是 delta 章节，因此会被忽略。请将其移到 "## ADDED Requirements"、"## MODIFIED Requirements"、"## REMOVED Requirements" 或 "## RENAMED Requirements" 之下。`,
          });
        }
        ...
        for (const block of plan.added) {
          ...
          if (scenarioCount < 1) {
            issues.push({ level: 'ERROR', path: entryPath, message: `ADDED "${block.name}" 必须至少包含一个场景${this.emptyScenarioHint(block.raw)}` });
          }
        }
        ...
        for (const block of plan.modified) {
          ...
          const scenarioCount = this.countScenarios(block.raw);
          if (scenarioCount < 1) {
            issues.push({ level: 'ERROR', path: entryPath, message: `MODIFIED "${block.name}" 必须至少包含一个场景${this.emptyScenarioHint(block.raw)}` });
          }
        }
    ...
    extractRequirementsSection(content).bodyBlocks.forEach((block, index) => {
      ...
      if (!requirementText) {
        issues.push({
          level: 'ERROR',
          path: `requirements[${index}]`,
          message: this.buildMissingShallOrMustMessage(`需求 "${block.name}"`, block.name),
        });
      } else if (!this.containsShallOrMust(requirementText)) {
        issues.push({
          level: 'WARNING',
  ...
  private containsShallOrMust(text: string): boolean {
    return containsShallOrMustShared(text) || /必须|禁止/.test(text);
  }
  ...
  private emptyScenarioHint(blockRaw: string): string {
    ...
    return ' （场景标题下没有正文不计入；请补充其步骤，例如 "- **WHEN** ..." 和 "- **THEN** ..."）';
  }
