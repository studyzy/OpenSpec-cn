...
export async function offerCliUpgrade(latestVersion: string): Promise<UpgradeOutcome> {
  ...
  if (!installed) {
    console.log(chalk.yellow('升级未能完成。全局安装可能需要'));
    console.log(chalk.yellow('提升权限，或者使用其他包管理器。'));
    return 'failed';
  }
  ...
  if (!version) {
    console.log(chalk.yellow('升级已完成，但找不到 "openspec" 来确认。'));
    return 'not-on-path';
  }
  if (compareVersions(version, OPENSPEC_VERSION) <= 0) {
    console.log(chalk.yellow(`升级已完成，但 "openspec" 仍显示 v${version}。`));
    console.log(
      chalk.dim(
        binPath
          ? // 我们直接查询了已安装的副本，所以问题不在 PATH。
            `  npm 报告成功，但 ${binPath} 未发生变化。`
          : '  你的 PATH 上更靠前的另一个安装先被找到。'
      )
    );
    return 'not-on-path';
  }
...
export async function rerunUpdateWithUpgradedCli(
  ...
  return new Promise((resolve) => {
    ...
    child.on('error', () => {
      ...
      console.log(chalk.yellow('指令文件未被重新生成。'));
      console.log(chalk.dim('  运行 "openspec-cn update" 以获取新的工作流。'));
      resolve(1);
    });
