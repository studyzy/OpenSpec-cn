export const agentsTemplate = `# OpenSpec 使用说明
...
## 修改需求
### 需求：现有功能
[完整的修改后需求] （注意：必须包含完整的需求描述和场景，不能只写标题！）
...
每个需求必须至少有一个场景。如果没有场景，验证将失败。

### 需求措辞
- **强制规则**：需求描述文本中**必须**包含以下关键词之一：\`必须\`、\`禁止\`、\`SHALL\`、\`MUST\`。
- 如果缺少这些关键词，验证工具将报错：\`必须包含 SHALL、MUST、必须 或 禁止\`。
- 避免使用\`应该\`、\`可能\`、\`should\`、\`may\`，除非该需求是非规范性的。
...
**重要提示**：
1. **必须使用正确的二级标题**：如 \`## 新增需求\`。不要使用 \`###\` 或其他格式。
2. **修改需求必须完整**：在 \`## 修改需求\` 下，必须提供**完整的**新需求内容（包括描述和所有场景）。绝不要只写标题！
3. **每个需求必须有场景**：每个需求下必须至少包含一个 \`#### 场景：\` 块。
...
4) **重要**：绝不要只写标题！必须包含完整的需求描述文本和场景。如果只写标题，验证工具将报错：\`缺少需求文本\`。
5) 确保标题文本完全匹配（空格不敏感）并保持至少一个\`#### 场景：\`。
...
**"必须包含 SHALL、MUST、必须 或 禁止"**
- 原因：需求描述中缺少规范性关键词。
- 修复：在需求描述中添加\`必须\`或\`禁止\`（或英文对应词）。
