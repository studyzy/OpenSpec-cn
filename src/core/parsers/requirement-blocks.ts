...
export function parseDeltaSpec(content: string): DeltaPlan {
  ...
  // English keys are passed here; getSectionsCaseInsensitive also accepts the
...
/** 四种规范化的 delta section 标题（英文 key）。 */
const CANONICAL_DELTA_SECTION_TITLES = [
  'ADDED Requirements',
...
/**
 * Every section title the delta reader acts on - English and its Chinese
 * aliases - folded the way `getSectionsCaseInsensitive` folds them. Matching
 * the reader exactly matters: a looser test (say, any run of whitespace) would
 * treat `## ADDED  Requirements` as a delta section here while the reader
 * ignores it, and the requirements under it would be dropped without this
 * warning. It must stay in step with `CN_DELTA_SECTION_ALIASES` too, or a
 * requirement under `## 新增需求` would be read normally and then reported as
 * orphaned.
 */
const DELTA_SECTION_TITLES = new Set(
  [
    ...CANONICAL_DELTA_SECTION_TITLES,
    ...Object.values(CN_DELTA_SECTION_ALIASES).flat(),
 ...
 *
 * Chinese delta section titles (新增需求 / 修改需求 / 移除需求 / 重命名需求) are
 * treated as aliases of the English keys, so both spellings apply in full.
 */
function getSectionsCaseInsensitive(
  ...
  const matches = sections.filter(
    (section) =>
      ...
      cnAliases.includes(section.title.toLowerCase())
  );
  if (matches.length === 0) {
...
function parseRemovedNames(sectionBody: SectionBody): string[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Also support bullet list of headers (English + Chinese). Every CommonMark
    // bullet marker counts: `*` and `+` open a list exactly as `-` does, so
    // accepting only `-` turned a removal written with either of them into a
    // silent no-op - archive reported success while the requirement stayed in
    // the spec.
...
function parseRenamedPairs(
  ...
  for (let i = 0; i < lines.length; i++) {
    if (fenceMask[i]) continue;
    const line = lines[i];
    // The bullet stays optional, any CommonMark marker is accepted (a rename
    // written with `*` or `+` used to match nothing at all, so the rename never
    // happened while archive still reported success), and the Chinese
    // `需求` spelling is accepted alongside `Requirement`.
...
export function describeScenarioBalance(diff: ScenarioNameDiff): string {
  const count = (value: number) => `${value} 个场景`;
  const scale = `修改后的块包含 ${count(diff.incomingCount)}；当前 spec 包含 ${count(diff.currentCount)}。`;
  if (diff.added.length === 0) {
    return `${scale} 它没有新增任何场景。`;
  }
  ...
  const names = rest > 0 ? `${listed} 以及另外 ${rest} 个` : listed;
  return `${scale} 它新增了当前 spec 中不存在的 ${count(diff.added.length)}：${names}。`;
}
