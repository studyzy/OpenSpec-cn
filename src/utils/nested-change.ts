 ...
 */
export const NESTED_CHANGE_ISSUE_MARKER = '不是一个变更：它是包裹着';
...
export function describeNestedChange(finding: NestedChangeFinding): string {
  ...
  return (
    ...
    '变更必须是 openspec/changes/ 下的直接子目录，因此这些嵌套目录对 OpenSpec ' +
    '不可见，而其外层文件夹会被当作一个变更报告。只有 openspec/specs/ 下' +
    `支持嵌套路径。请将每个嵌套变更重命名为扁平名称（例如 "${example}"）。`
  );
}
