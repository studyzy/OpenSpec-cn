...
这些应该小、清晰、逻辑顺序合理。每个任务组自带其工作所需的测试和文档 - 最后一个任务组只用于集成检查。
```
...
- [ ] 1.1 [具体任务] — verify: [测试、命令、可观察行为或交付的产物]
- [ ] 1.2 [具体任务] — verify: [测试、命令、可观察行为或交付的产物]

## 2. 集成验证

- [ ] 2.1 使用[端到端测试或可观察结果]验证[更广泛的集成或系统行为]

---

每个复选框成为 apply 阶段的工作单元。这个任务拆分看起来合适吗？
```

**暂停** - 等待用户批准/反馈。

批准后，保存到 `openspec-cn instructions tasks --change "<name>" --json` 的 `resolvedOutputPath`。
